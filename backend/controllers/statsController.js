...
export const getPlatformStats = async (req, res) => {
  try {
    ...
    const settledQuestions = questions.filter(q => q.status === 'settled').length;
    ...
    const totalOcroWithdrawn = withdrawals.reduce(
      (sum, w) => sum + parseFloat(w.ocro_amount),
      0
    );
...
export const getUserStats = async (req, res) => {
  try {
    ...
    const bets = await Bet.find({ user_address: userAddress.toLowerCase() }).lean();
    ...
    const withdrawals = await Withdrawal.find({ user_address: userAddress.toLowerCase() }).lean();

    const totalBets = betsWithQuestions.length;
    const activeBets = betsWithQuestions.filter(b => b.questions && b.questions.status === 'open').length;
    const settledBets = betsWithQuestions.filter(b => b.questions && b.questions.status === 'settled').length;
    const wonBets = betsWithQuestions.filter(b => b.is_winner === true).length;
    const lostBets = betsWithQuestions.filter(b => b.is_winner === false).length;
    ...
    const stats = {
      bets: {
        ...
        won: wonBets,
        lost: lostBets,
        winRate: settledBets > 0 ? ((wonBets / settledBets) * 100).toFixed(2) : 0
      },
      staked: {
        totalOcroStaked,
        totalUsdtStaked
      ...
      profit: {
        ocroProfit: totalOcroWithdrawn - totalOcroStaked,
...
export const getLeaderboard = async (req, res) => {
  try {
    ...
    const withdrawals = await Withdrawal.find({}).select('user_address ocro_amount usdt_amount').lean();
    ...
    withdrawals.forEach(w => {
      ...
      if (userMap.has(addr)) {
        const user = userMap.get(addr);
        user.totalWithdrawn += parseFloat(w.ocro_amount) + parseFloat(w.usdt_amount);
      }
    });
    ...
    const leaderboard = Array.from(userMap.values())
      .map(user => ({
        ...user,
        winRate: user.totalBets > 0 ? ((user.wonBets / user.totalBets) * 100).toFixed(2) : 0
      }))
