...
export const recordBet = async (req, res) => {
  try {
    ...
    if (stats) {
      if (outcome === 'yes') {
        stats.yes_ocro_total = parseFloat(stats.yes_ocro_total) + 1;
        stats.yes_usdt_total = parseFloat(stats.yes_usdt_total) + 1;
        stats.yes_participants = stats.yes_participants + 1;
      } else {
        stats.no_ocro_total = parseFloat(stats.no_ocro_total) + 1;
...
export const calculateWinnings = async (req, res) => {
  try {
    ...
    if (!question.contract_question_id) {
      return res.status(400).json({ success: false, error: 'Question not on blockchain' });
    }
    ...
    res.json({
      success: true,
      data: {
        ocroWinnings: ethers.formatEther(ocroWinnings),
...
export const recordWithdrawal = async (req, res) => {
  try {
    const { questionId, userAddress, ocroAmount, usdtAmount, transactionHash } = req.body;
    ...
    const existingWithdrawal = await Withdrawal.findOne({ transaction_hash: transactionHash });
...
export const getUserWithdrawals = async (req, res) => {
  try {
    ...
    const withdrawals = await Withdrawal.find({ user_address: userAddress.toLowerCase() })
      .sort({ created_at: -1 })
      .lean();
